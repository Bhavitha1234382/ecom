...
mydb=mysql.connector.connect(user='root',host='localhost',password='@Bhavi123',db='ecom')
@app.route('/')
def home():
...
def readreview(itemid):
    if session.get('user'):
        try:
            ...
            return render_template('readreview.html',itemid=itemid,data=data,item_data=item_data)
    else:
        flash(f'pls login first')
        return redirect(url_for('userlogin'))
